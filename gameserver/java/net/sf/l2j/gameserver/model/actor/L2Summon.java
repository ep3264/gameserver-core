...
public abstract class L2Summon extends L2Playable
{
	...
	public boolean useMagic(L2Skill skill, boolean forceUse, boolean dontMove)
	{
		...
		if (skill.isOffensive())
		{
			...
			{
				canAttackInPeace=EventManager.getInstance().getCurrentEvent().canAttackInPeace(getOwner(),(L2Character) target);
			}
	...
	public void doCast(L2Skill skill)
	{
		...
		if (EventManager.getInstance().getCurrentEvent() != null)
		{
			canAttackInPeace=EventManager.getInstance().getCurrentEvent().canAttackInPeace(actingPlayer,(L2Character) actingPlayer.getTarget());
		}
		if (!actingPlayer.checkPvpSkill(getTarget(), skill) && !actingPlayer.getAccessLevel().allowPeaceAttack()&& !canAttackInPeace)
		{

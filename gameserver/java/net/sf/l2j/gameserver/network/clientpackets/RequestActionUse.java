		...
		switch (_actionId)
		{
			...
			case 22: // Attack (pet attack)
				...
				else if (pet.getNpcId() != L2SiegeSummonInstance.SIEGE_GOLEM_ID)
				{
					...
					{
						canAttackInPeace=EventManager.getInstance().getCurrentEvent().canAttackInPeace(pet.getOwner(),(L2Character) target);
					}
					if (L2Character.isInsidePeaceZone(pet, target) && !canAttackInPeace)
					{
						pet.setFollowStatus(false);
						pet.getAI().setIntention(CtrlIntention.FOLLOW, target);
					}

...
public final class L2GameClient extends MMOClient<MMOConnection<L2GameClient>> implements Runnable
{
	...
	private boolean _preAuth = true;
	@SuppressWarnings("unused")
	private boolean _isAuthedGG;
	private final long _connectionStartTime;
	private CharSelectInfoPackage[] _charSlotMapping = null;
	private MultiValueSet<String> _accountData = new MultiValueSet<>();
	...
	public void setAccountName(String pAccountName) throws SQLException
	{
		...
		try
		{
			...
			stm = con.prepareStatement(LOAD_ACC_DATA);
		...
		{
			_log.warning("Error restore account data, for account " + pAccountName + ". Error" + e.toString());
		}
		finally
		{
			if (rs != null)
	...
	public void storeData() {
		try
		{
			...
			for(String s : _accountData.keySet())
			{
		...
		{
			_log.warning("Error while saving account data"+ e);
		}
	...
	private ReceivablePacket<L2GameClient> _prevPacket = null;
	private boolean _pressedSkill = false;
	
	/**
	 * Fixed: Redist Add packet to the queue and start worker thread if needed
	 * @param packet The packet to execute.
	 */
	public void execute(ReceivablePacket<L2GameClient> packet)
	{
		// pressed fix
		if (packet instanceof RequestMagicSkillUse && packet.equals(_prevPacket))
		{
			_pressedSkill = true;
			if (getActiveChar().isCastingNow())
			{
				return;
			}
		}
